...
1) Entry Point – Program Trigger

Execution begins inside bcpnn.py at line 67 where we start timing:
...
Intent:
Convert raw reports into a mathematical structure usable for probabilistic scoring.

4) Parameter Retrieval + Priors Load

Handled by function:

read_parameters("./outputs/alpha.csv","./outputs/beta.csv","./outputs/gamma.csv")
...
The classification happens here inside main loop:
...
Located in bcpnn.py line 51

write_out(a_parameters,b_parameters,g_parameters,"./outputs/alpha.csv",...)
